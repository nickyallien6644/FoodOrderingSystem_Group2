...
public class SearchForDrinkActivity extends AppCompatActivity {
    ...
    List<Product> listProductSearchForDrink;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listProductSearchForDrink = new ArrayList<>();
        getResultForDrink(listProductSearchForDrink);
    ...
    public void getResultForDrink(List<Product> listProductSearchForDrink) {
        apiInterface = Api.getClients();

        resultSearch = new ArrayList<>();
        Intent intent = getIntent();
        if (intent.getStringExtra("searchForDrink") != null) {
            search = intent.getStringExtra("searchForDrink");
        }
        ...
        call.enqueue(new Callback<List<Product>>() {
            ...
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                ...
                if (!search.equals(null)) {
                    ...
                    for (Product product : resultSearch) {
                        if (product.getpName().toLowerCase().contains(search.toLowerCase())) {
                            listProductSearchForDrink.add(product);
                            checkResult = true;
                        }
                    }
                    if (listProductSearchForDrink.size() == 0) {
                        Toast.makeText(SearchForDrinkActivity.this, "Couldn't find the matching result you were searching for!", Toast.LENGTH_SHORT).show();
                    }
                }
                ...
                if (checkResult == true) {
                    searchResult(listProductSearchForDrink);
                    checkResult = false;
                }
